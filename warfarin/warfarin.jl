...
# Main algorithms: FO(), FOCE(), LaplaceI()
...
# Save a fitted Pumas model
using Serialization
filename = "warfarin_fpm.jls"
serialize(filename, fpm)

# Load a saved fitted Pumas model
saved_fpm = deserialize(filename)
...
inf = infer(fpm) # fails
# Bootstrap - using 5 samples just to demonstrate
...
sims4 = simobs(bts_inf, pop, samples = 1000)
